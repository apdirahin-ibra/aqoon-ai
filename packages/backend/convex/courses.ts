...
// ─── List all students across tutor's courses ────────────────────────────────
export const tutorStudents = query({
  args: {},
  handler: async (ctx) => {
    const { user } = await requireTutor(ctx);
    ...
    for (const course of courses) {
      ...
      for (const enrollment of enrollments) {
        ...
        const courseCompletedCount = completedLessons.filter((lp) =>
          totalLessons.some((tl) => tl._id === lp.lessonId),
        ).length;

        const progress =
          totalLessons.length > 0
            ? Math.round((courseCompletedCount / totalLessons.length) * 100)
            : 0;
